...
print("package_version=1.0.0rc1 build_backend=setuptools==84.0.0 build_frontend=build==1.3.0")
PY
...
tar -tzf release/dist/relay_hermes-1.0.0rc1.tar.gz |
  grep -E '/(plugin.yaml|scripts/check-openapi.py|scripts/run-staging.sh|tests/test_plugin_manifest.py)$'
if tar -tzf release/dist/relay_hermes-1.0.0rc1.tar.gz | grep -q '/contracts/'; then
  echo 'contract snapshot unexpectedly shipped in package' >&2
  exit 1
fi
unzip -l release/dist/relay_hermes-1.0.0rc1-py3-none-any.whl
(
  cd release
...
echo retained_distribution_pair_build_count=1
